...
const DOCUMENT_BUCKET = "expense-documents",
...
export async function POST(request: NextRequest) {
  ...
  const file = form.get("file"),
    ...
    plan = z.uuid().safeParse(form.get("expense"));
  if (
  ...
  const { error: finalizeError } = await db.rpc("finalize_expense_document", {
  ...
  revalidatePath(`/pets/${pet.data}/costs/expenses/${plan.data}`);
  return NextResponse.json(
    { id: document.id },
